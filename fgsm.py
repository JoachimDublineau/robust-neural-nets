...
parser.add_argument(
    "--output-csv",
    "-csv",
    default="{}fgsm_evaluation_log.csv".format(src.results_dir),
    help='the path of the csv file in which the script writes its outputs. Default is "{}fgsm_evaluation_log.csv"'.format(
        src.results_dir
    ),
)
parser.add_argument(
...
parser.add_argument(
    ...
    default="3",
    choices=["0", "1", "2", "3"],
    help="Tensorflow minimum cpp log level. Default is 3",
)
...
signed_gradients = src.attacks.compute_signed_gradients(
    ...
    y_test,
    model,
    klosses.categorical_crossentropy,
...
with open(args.output_csv, "w") as output:
    ...
    for epsilon in args.epsilons:
        ...
        evaluation = model.evaluate(x, y_test, batch_size=args.batch_size, verbose=0)
        output.write(
            "{};{}\n".format(
                epsilon,
                ";".join(map(lambda number: "{:.5f}".format(number), evaluation)),
            )
        )
        ...
        if args.save_img is not None:
            ...
            for image_index in iterator:
                ...
                axes[image_index][0].set_title(
                    "{}: {:.4f}".format(
                        src.cifar10.labels[np.argmax(y_predicted[image_index])],
                        np.max(y_predicted[image_index]),
                    ),
                    fontdict=font,
                )
                axes[image_index][1].imshow(gradients[image_index])
                axes[image_index][1].set_title("Signed gradient", fontdict=font)
                axes[image_index][2].imshow(x_adversarials[image_index])
                axes[image_index][2].set_title(
                    "{}: {:.4f}".format(
                        src.cifar10.labels[
                            np.argmax(y_adversarials_predicted[image_index])
                        ],
                        np.max(y_adversarials_predicted[image_index]),
                    ),
                    fontdict=font,
                )
            ...
            fig.savefig(
                "{}_{}_image.png".format(args.output_csv, epsilon),
                dpi=400,
                transparent=True,
            )
