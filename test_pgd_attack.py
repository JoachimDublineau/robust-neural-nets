...
perturbation = generate_pgd_attack(model, categorical_crossentropy, image, label, eps=1)
print("Perturbation:")
# print(perturbation)
...
print("Model prediction:", np.argmax(model(K.cast([image], dtype="float32"))[0]))
print()
print("Perturbated image:")
print("Norm:", np.linalg.norm(image + perturbation))
print(
    "Model prediction:",
    np.argmax(model(K.cast([image + perturbation], dtype="float32"))[0]),
)
print()
