...
parser = argparse.ArgumentParser(description="Script to create graphs from fgsm logs")
parser.add_argument(
    "path",
    help="path to the fgsm logs files. It is assumed that the files have the same headers",
    nargs="+",
)
parser.add_argument(
    "-o",
    "--output",
    help="Path of the ouput file. Default is {}fgsm_graphs.png".format(src.results_dir),
    type=str,
    ...
    for metric_index, metric in enumerate(data.columns[1:]):
        if metric != epsilon:
            axes[metric_index].plot(
                data[epsilon],
                data[metric],
                label=None if args.labels is None else args.labels[index],
                alpha=0.7 if args.labels is not None else 1.0,
            )
